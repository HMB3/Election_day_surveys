...
venue_seifa <- dplyr::select(respondent_index_venue_average,
                             Venue_Voting_Load, 
  ...
  mutate(Informality        = as.numeric(Informality),
         Venue_Voting_Load  = as.numeric(Venue_Voting_Load),
...
venue_seifa_WHS <- dplyr::select(respondent_index_venue_average,
                                 WHS,
                                 Respondent_Satisfied,
                                 Respondent_Dissatisfied,
                                 Venue_Voting_Load, 
...
venue_satisf_index <- dplyr::select(respondent_index_venue_average,
                                    Respondent_Satisfied,
                                    Counting,
                                    IS,
                                    Logistics,
...
## What Venues/LGAs are missing ABS context data?
summary(venue_seifa_WHS);summary(venue_satisf_index)


## Create table of venue indexes (just the index questions) and overall dissatisfaction (all questions)
venue_unsatisf_index <- dplyr::select(respondent_index_venue_average,
                                      Respondent_Dissatisfied,
                                      Counting,
                                      IS,
                                      Logistics,
...
venue_work_ses_remotess_plot <- scatter_matrix_grouped_histo(scat_df     = venue_seifa, 
                                                             cols        = 1:6, 
                                                             scat_col    = 'Remoteness',
                                                             alpha       = 0.5, 
                                                             alignPer    = 0.8,
...
venue_work_ses_remotess_WHS_plot <- scatter_matrix_grouped_histo(scat_df     = venue_seifa_WHS %>% dplyr::select(-Informal),
                                                                 cols        = 1:6,
                                                                 scat_col    = 'Remoteness',
                                                                 alpha       = 0.5, 
                                                                 alignPer    = 0.8,
...
venue_satisfaction_vs_indexes   <- psych::pairs.panels(venue_satisf_index,
...
venue_dissatisfaction_vs_indexes <-pairs.panels(venue_unsatisf_index,
                                                method   = "pearson", # correlation method
                                                hist.col = "#00AFBB",
                                                density  = TRUE,      # show density plots
                                                ellipses = FALSE,
...
venue_index_load_plot_df = dplyr::select(respondent_index_venue_average, 
                                         -VenueName) %>%
...
## Create DF for venue-level respondents
venue_index_context_plot_df = dplyr::select(respondent_index_venue_average,
                                            -VenueName) %>%
...
venue_EO_index   <- respondent_index_ro_join %>% filter(Role == "EO") %>% 
  
  group_by(VenueName) %>% 
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE))) %>% 
  select(VenueName, index_plot_columns) %>% 
  
  ## Join on the context venue columns
  right_join(venue_workload_satisfaction, ., by = "VenueName")

venue_OA_index   <- respondent_index_ro_join %>% filter(Role == "OA") %>% 
...
venue_DPM_index  <- respondent_index_ro_join %>% filter(Role == "DPPM") %>% 
...
venue_PPM_index  <- respondent_index_ro_join %>% filter(Role == "PPM") %>% 
...
venue_SOA_index  <- respondent_index_ro_join %>% filter(Role == "SOA") %>% 
...
venue_SOAP_index <- respondent_index_ro_join %>% filter(Role == "SOAPP") %>% 
...
venue_plot_overall  <- c("respondent_index_venue_average",
                         "venue_OA_index", 
                         "venue_EO_index", 
                         "venue_SOA_index", 
                         "venue_SOAP_index", 
...
venue_load_scatterplots <- venue_plot_overall %>%
...
venue_context_scatterplots <- venue_plot_overall %>%
...
names(venue_load_scatterplots)    <- c("venue_load_plot_df_numeric",
                                       "venue_OA_load_plot_df", 
                                       "venue_EO_load_plot_df", 
                                       "venue_SOA_load_plot_df", 
                                       "venue_SOAP_load_plot_df", 
                                       "venue_PPM_load_plot_df", 
                                       "venue_DPM_load_plot_df")


names(venue_context_scatterplots) <- c("venue_context_plot_df",
                                       "venue_OA_context_plot_df", 
                                       "venue_EO_context_plot_df", 
                                       "venue_SOA_context_plot_df", 
                                       "venue_SOAP_context_plot_df", 
                                       "venue_PPM_context_plot_df", 
                                       "venue_DPM_context_plot_df")


list2env(venue_load_scatterplots,    globalenv())
list2env(venue_context_scatterplots, globalenv())
...
venue_index_load_names <- mget(ls(name = .GlobalEnv, pattern = 'load_plot_df',    mode == "symbol")) %>% 
  names() %>% sort()
venue_context_names    <- mget(ls(name = .GlobalEnv, pattern = 'context_plot_df', mode == "symbol")) %>% 
  names() %>% sort() 
...
scatterplot_venue_indexes_workload <- 
  
  scatter_keyvar_ungroup_list(scatplot_list      = names(venue_load_scatterplots)[-1], 
                              ...
                              ## Names
                              scatplot_names = names(venue_load_scatterplots)[-1])
...
scatterplot_venue_indexes_context_HSES <- 
  
  scatter_keyvar_ungroup_list(scatplot_list  = names(venue_context_scatterplots), 
                              ...
                              ## Names
                              scatplot_names = names(venue_context_scatterplots))
...
scatterplot_venue_indexes_context_LSES <- 
...
## Response Indexes vs. Low SES ----
scatterplot_venue_indexes_context_CALD <- 
...
## Index vs. workload for venues which were under-projected
venue_under_projected_v_work <- 
  
  scatter_matrix_keyvar(scat_df <- venue_index_load_plot_df %>%
...
venue_index_correlations <- 
  
  scatter_matrix_grouped_histo(scat_df <- venue_index_load_plot_df %>%
...
venue_index_correlations_remote <- 
  ...
  scatter_matrix_grouped_histo(scat_df <- venue_context_plot_df %>% 
...
## Then plot the relevant plots at the RO level
