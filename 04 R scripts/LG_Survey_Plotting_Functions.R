...
# 
# ymin  = 0
# ymax = max(df$Percentage) + max(df$Percentage*0.2)
# ylab  = ylab
# xlab  = ''

## Bar chart horizontal ---- 
single_barchart_order_y = function(df, title, ylab, xlab, ymin, ymax, axis_multiplier, mar,
                                   tsize, caption, xsize, ysize, ycol, 
                                   lab_size, bar_width, capt_size, color_n) {
  
  ## Set the max and min inside function?
  ymax = max(df$Percentage) + max(df$Percentage * axis_multiplier)
  
  plot <- ggplot(df, aes(x = reorder(Response, Percentage), 
...
single_bar_order_factor_facet = function(df, title, Response, ylab, xlab, ymin, ymax, axis_multiplier, col_palette,
  ...
  plot <- ggplot(df, aes(x = !!sym(Response), y = Percentage, fill = !!sym(Response))) +
    geom_bar(stat = "identity", position = "dodge") +
    coord_flip() +
...
zero_centred_barchart = function(highs,  lows, caption, capt_size,
  ...
  ggplot() + 
    ...
    geom_text(data = high_label, aes(x = Role, y = Percentage, label = Percentage), size = lab_size, hjust =  high_just) +
    geom_text(data = low_label,  aes(x = Role, y = Percentage, label = Percentage), size = lab_size, hjust =  low_just) +
...
## Simple percentage bar function ---- 
