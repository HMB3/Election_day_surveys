...
The following analysis is based on voting opertions questions in the LG21 staff survey, using only complete responses 
...
#### **Fig A). Survey responses, aggregated by all topics (left), and by role for voting operations questions (right. Note ROs were excluded from the index analysis)**

```{r fig.align = "center", fig.width = 15, fig.height = 7}

## Arrange a grid
plot_grid(bp_resposes_all, 
          respondent_role_boxplots[["Operations_boxplot"]],
          nrow = 1,
...
#### **Fig B). Survey responses by demographics and remoteness for voting operations questions (note, categories are not necessarily mutually exclusive)**

```{r fig.align = "center", fig.width = 15, fig.height = 7}

## Arrange a grid
plot_grid(respondent_demo_boxplots[["Operations_boxplot"]], 
          respondent_remoteness_boxplots[["Operations_boxplot"]],
          nrow = 1,
          label_size = 5, 
          align = 'hv')
...
#### **Fig C). Voting opertions survey responses by RO Region**

The map below shows the index of survey responses for voting opertions questions in the LG21 staff survey. The underlying data are the same index
...
RO_region_polygon_index_maps[["Logistics_polygon_map"]]
